  ...
  const loadSingleImage = async (item, retryCount = 0) => {
    ...
    if (stages && Array.isArray(stages) && stages.length > 0) {
      ...
      return loadImageProgressiveWithCache(url, {
        ...
        onStageError: onStageError, // 传递阶段错误回调
        ...
        const result = {
          ...
          fromCache: progressiveResult.fromCache || false, // 标记是否来自缓存
        };
  ...
  try {
        ...
        stageResult = await new Promise((resolve, reject) => {
          ...
          img.onerror = (event) => {
            if (!isResolved) {
              ...
              cleanup();
              // 尝试从错误事件中获取更多信息
              let errorMessage = `阶段 ${i + 1} 加载失败`;
              try {
                // 检查是否是网络错误或 404
                if (event.target && event.target.src) {
                  const url = event.target.src;
                  // 不显示具体的 URL，避免控制台噪音
                  errorMessage = `阶段 ${i + 1} 加载失败`;
                }
              } catch (e) {
                // 忽略
              }
    ...
    if (!hasSuccess) {
      ...
      const is404 = lastError.message.includes('404') || 
                   (lastError.originalEvent && lastError.originalEvent.type === 'error');
      if (!is404 || process.env.NODE_ENV === 'development') {
        if (!is404) {
          console.error(`❌ 图片加载失败: ${url.substring(0, 50)}...`, lastError.message);
        }
      }
    ...
    if (!is404 || process.env.NODE_ENV === 'development') {
      if (!is404) {
        console.error(`❌ 渐进式加载过程出错: ${url.substring(0, 50)}...`, error.message || error);
      }
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const compressedFileName = buildOutputFileName(sourceFileName, outputFormat, mimeType);
