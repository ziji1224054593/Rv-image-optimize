...
function buildVariantAssetName(assetName, variant) {
  ...
  return path.posix.join(parsed.dir, `${parsed.name}${suffix}.${extension}`);
}
...
async function optimizeRasterAsset(assetName, extension, inputBuffer, options) {
  ...
  for (const variant of options.variants) {
    const variantResult = await compressImageBuffer(inputBuffer, {
    ...
    const savedSize = inputBuffer.length - variantResult.compressedSize;
    const minSavings = Math.max(options.variantMinSavings, variant.minSavings || 0);
    if (savedSize <= minSavings) {
      continue;
    }

    item.generatedVariants.push({
      assetName: buildVariantAssetName(assetName, variant),
      format: variant.format,
      buffer: variantResult.buffer,
      compressedSize: variantResult.compressedSize,
      savedSize: Math.max(savedSize, 0),
...
async function processAssets(compilation, compiler, options) {
  ...
  const items = await runWithConcurrency(targetAssets, options.concurrency, async ({ name, source }) => {
    ...
    try {
      ...
      if (optimized.item.generatedVariants?.length) {
        optimized.item.generatedVariants.forEach((variant) => {
          updateAsset(compilation, compiler, variant.assetName, variant.buffer);
        });
      }
  ...
  if (options.manifest) {
    const manifest = items.reduce((result, item) => {
      ...
      result[item.assetName] = {
        original: item.assetName,
        variants: (item.generatedVariants || []).map((variant) => ({
          path: variant.assetName,
          format: variant.format,
    ...
    summary.manifestAsset = options.manifestFile;
  }
