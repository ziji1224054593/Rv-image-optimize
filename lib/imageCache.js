...
import { destroyWorker, sendMessage, shouldUseWorker } from './imageCacheManager.js';
...
export const DEFAULT_CACHE_KEY_PREFIX = 'image';
...
function normalizeCacheUrl(url, options = {}) {
  ...
  try {
    const urlObject = new URL(url, typeof window !== 'undefined' ? window.location.origin : 'https://example.com');
    const keysToDrop = new Set((Array.isArray(dropParams) ? dropParams : [dropParams]).filter(Boolean).map((item) => String(item)));

    const nextSearchParams = new URLSearchParams();
    const entries = Array.from(urlObject.searchParams.entries())
      .filter(([key]) => !keysToDrop.has(key));
    ...
    urlObject.search = nextSearchParams.toString();

    return `${urlObject.origin}${urlObject.pathname}${urlObject.search ? `?${urlObject.search}` : ''}${urlObject.hash}`;
  } catch (error) {
    return url;
  }
}

export function normalizeImageCacheUrl(url, options = {}) {
  return normalizeCacheUrl(url, options);
}

export function createImageCacheKey(url, options = {}) {
  const {
  ...
  const normalizedUrl = normalizeKey ? normalizeCacheUrl(url, { dropParams, sortQuery }) : url;
  return `${keyPrefix}:${cacheVersion}:${normalizedUrl}`;
}
...
export function resetImageCacheMetrics() {
  Object.keys(imageCacheMetrics).forEach((key) => {
    imageCacheMetrics[key] = 0;
  });
}

function parseImageCachePayload(cached) {
  ...
  return {
    bytes,
    blobUrl: createBlobUrlFromCache(bytes, cached.mimeType),
  };
}

function toBase64DataUrl(imageData, mimeType) {
  const chunks = [];
...
function shouldRefreshInBackground(cached, staleHours) {
  if (!cached?.cachedAt || !staleHours || staleHours <= 0) {
    return false;
  }
  const staleMs = staleHours * 60 * 60 * 1000;
  return (Date.now() - Number(cached.cachedAt)) >= staleMs;
}
 ...
 * 删除整个数据库
 * @param {string} dbName - 数据库名称（库名）
 * @description 删除前会尽量关闭当前缓存的数据库连接；如果浏览器报告 blocked，会等待连接释放；
 * 超时后抛出明确错误，避免调用方无限等待。
 * @returns {Promise<void>}
 */
export const deleteDatabase = async (dbName) => {
  try {
    if (shouldUseWorker()) {
      try {
        await sendMessage('closeDatabaseConnections', { dbName });
      } finally {
        destroyWorker();
      }
    }

    return await mainThreadCache.mainThreadDeleteDatabase({ dbName, timeoutMs: 10000 });
  } catch (error) {
...
export const loadImageWithCache = async (
  ...
  const resolvedOptions = typeof expireHours === 'object' && expireHours !== null
    ? expireHours
    : { expireHours };
  const {
    expireHours: resolvedExpireHours = DEFAULT_CACHE_EXPIRE_HOURS,
    staleHours = 0,
    cacheVersion = DEFAULT_CACHE_VERSION,
    keyPrefix = DEFAULT_CACHE_KEY_PREFIX,
    normalizeKey = true,
  ...
  const fetchAndCache = async (refreshInBackground = false) => {
    const inFlightKey = `${dbName}:${storeName}:${cacheKey}`;
    if (imageLoadInFlight.has(inFlightKey)) {
      imageCacheMetrics.inflightHits += 1;
      return imageLoadInFlight.get(inFlightKey);
    }
    ...
    const promise = (async () => {
      ...
      const dataUrl = toBase64DataUrl(imageData, mimeType);
      ...
      try {
        await setCache(
          cacheKey,
          {
            ...
            cachedAt: Date.now(),
            originalUrl: url,
            normalizedUrl: normalizeKey ? normalizeCacheUrl(url, { dropParams, sortQuery }) : url,
          },
          resolvedExpireHours,
          dbName,
          storeName
        );
        imageCacheMetrics.writes += 1;
        if (refreshInBackground) {
          imageCacheMetrics.revalidations += 1;
        }
      } catch (saveError) {
    ...
    imageLoadInFlight.set(inFlightKey, promise);
    try {
      return await promise;
    } finally {
      imageLoadInFlight.delete(inFlightKey);
    }
  };
  ...
  try {
    ...
    if (cached && cached.data && cached.mimeType) {
      try {
        const { bytes, blobUrl } = parseImageCachePayload(cached);
        if (blobUrl) {
          imageCacheMetrics.hits += 1;
          imageCacheMetrics.bytesServedFromCache += bytes.byteLength;

          if (shouldRefreshInBackground(cached, staleHours)) {
            imageCacheMetrics.staleHits += 1;
    ...
    imageCacheMetrics.misses += 1;
    return await fetchAndCache(false);
  } catch (error) {
...
export const loadImageProgressiveWithCache = async (url, options = {}) => {
  ...
  try {
    ...
    // 2. 检查缓存（使用通用 API）
    const cacheKey = createImageCacheKey(finalUrl, {
      keyPrefix,
      cacheVersion,
      normalizeKey,
      dropParams,
    ...
    if (cached && cached.data && cached.mimeType) {
      try {
        ...
        if (blobUrl) {
          ...
          if (shouldRefreshInBackground(cached, staleHours)) {
            imageCacheMetrics.staleHits += 1;
            loadImageWithCache(url, dbName, storeName, {
              expireHours,
              staleHours: 0,
              cacheVersion,
              keyPrefix,
    ...
    const result = await loadImageProgressive(url, {
      ...
      onComplete: async (loadedUrl) => {
        if (!loadedUrl.startsWith('blob:')) {
          try {
            const response = await fetch(loadedUrl, { signal });
            if (response.ok) {
              const arrayBuffer = await response.arrayBuffer();
              const imageData = new Uint8Array(arrayBuffer);
              const mimeType = response.headers.get('Content-Type') || 'image/jpeg';
              ...
              try {
                ...
                await setCache(
                  cacheKey,
                  {
                    ...
                    cachedAt: Date.now(),
                    originalUrl: url,
                    normalizedUrl: normalizeKey ? normalizeCacheUrl(finalUrl, { dropParams, sortQuery }) : finalUrl,
                  },
                  expireHours,
                  dbName,
                  storeName
